...
export function SettingsPage({ onBack }: SettingsPageProps) {
  ...
  const [accessProjects, setAccessProjects] = useState<string[]>([]);
  ...
  function openManage(provider: ProviderConfig) {
    ...
    getProviderProjects(provider.id)
      .then(projects => setAccessProjects(projects))
      .catch(() => setAccessError('Unable to verify access. Your token may have expired.'))
    ...
    <div className="max-w-2xl mx-auto px-6 py-8">
      ...
      {activeView === 'manage' && activeProvider && activeConnection && (
        <div className="card p-6">
          ...
          {/* Access & Permissions */}
          <div className="mb-6">
            <h4 className="text-xs font-semibold text-slate-700 mb-3 uppercase tracking-wide">Your Access</h4>
            {accessLoading && (
              <p className="text-xs text-slate-400">Checking access...</p>
            )}
            {accessError && (
              <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-2">
                <p className="text-xs text-red-700">{accessError}</p>
              </div>
            )}
            {!accessLoading && !accessError && accessProjects.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xs text-slate-500">Projects you can access:</span>
                  <span className="px-2 py-0.5 bg-green-100 text-green-700 text-[10px] font-medium rounded-full">Read & Write</span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {accessProjects.map(project => (
                    <span key={project} className="px-2.5 py-1 bg-slate-100 text-slate-700 text-xs rounded-lg">
                      {project}
                    </span>
                  ))}
                </div>
            ...
            {!accessLoading && !accessError && accessProjects.length === 0 && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg px-3 py-2">
                <p className="text-xs text-yellow-800">No projects found. Your PAT may not have project-level access.</p>
              </div>
            )}
          </div>
